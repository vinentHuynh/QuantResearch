"""Fully re-simulate Sarah Phase 3 candidates one rule at a time.
...
Run from the repository root:
    ...
    .venv/bin/python scripts/mnq/sarah_phase4_backtest.py
"""
...
import sarah_baseline_backtest as baseline
...
from sarah_phase3_screen import month_block_interval
...
DEFAULT_PHASE1 = ROOT / "reports" / "sarah_baseline"
DEFAULT_PHASE2 = ROOT / "reports" / "sarah_phase2"
DEFAULT_PHASE3 = ROOT / "reports" / "sarah_phase3"
DEFAULT_OUTPUT = ROOT / "reports" / "sarah_phase4"
...
VARIANTS = [
    ...
    Variant(
        "1h · first Sarah test", "05_1h_first_sarah_test",
        "A zone is eligible only before its first selected Sarah test.", "first_sarah_test",
    ),
    Variant(
...
def make_filter(
    ...
    def entry_filter(
        ...
        if variant.rule == "first_sarah_test":
...
def write_report(
    ...
    best = verdicts.loc[verdicts.variant != "Sarah baseline"].sort_values(
    ...
    <title>Sarah Phase 4 full simulations</title><style>{STYLE}</style></head><body><main class='wrap'>
    <div class='eyebrow'>MNQ · Sarah strategy lab</div><h1>Phase 4 full strategy simulations</h1>
    <p class='lead'>Each Phase 3 candidate is independently re-run through Sarah's one-minute engine. Zone priority, skipped entries, replacement signals, rearming, stops, targets, and timed closes are recalculated.</p>
    ...
    <p class='note'><a href='../sarah_dashboard/index.html'>Main dashboard</a> · <a href='../sarah_phase3/index.html'>Phase 3 screen</a></p>
...
def main() -> None:
    ...
    series: dict[str, pd.DataFrame] = {"Sarah baseline": control}
