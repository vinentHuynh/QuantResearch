"""Build an offline HTML dashboard for Sarah strategy experiments.
...
Create/update the baseline dashboard:
    ...
    .venv/bin/python scripts/mnq/sarah_html_report.py

Add a future experiment and retain all existing lines:

    .venv/bin/python scripts/mnq/sarah_html_report.py \
        --series "BOS filter=reports/sarah_bos/trades.parquet"
"""
...
DEFAULT_OUTPUT = ROOT / "reports" / "sarah_dashboard"
DEFAULT_BASELINE = ROOT / "reports" / "sarah_baseline" / "trades.parquet"
DEFAULT_WINDOW = ROOT / "reports" / "sarah_baseline_2026_window" / "trades.parquet"
DEFAULT_PHASE2 = ROOT / "reports" / "sarah_phase2" / "summary.json"
DEFAULT_PHASE3 = ROOT / "reports" / "sarah_phase3" / "summary.json"
DEFAULT_PHASE4 = ROOT / "reports" / "sarah_phase4" / "summary.json"
DEFAULT_PHASE5 = ROOT / "reports" / "sarah_phase5" / "summary.json"
DEFAULT_PHASE6 = ROOT / "reports" / "sarah_phase6" / "summary.json"
TZ = "America/Chicago"
COLORS = [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reset-series",
        action="store_true",
        help="reset the registry to the Sarah baseline before applying --series",
    )
...
def update_registry(args: argparse.Namespace) -> dict[str, str]:
    ...
    if args.reset_series or not registry_path.exists():
        registry = {"Sarah baseline": portable_path(DEFAULT_BASELINE)}
    else:
...
def window_panel() -> str:
    ...
    <section>
      ...
      <p class="note">This short window is shown for comparison with Sarah's configured date range. It is not used as a separate strategy line.</p>
    </section>"""
...
def phase2_panel() -> str:
    ...
    <section>
      ...
      <p class="note">This is the point-in-time feature and outcome-label layer, not a new strategy line. <a href="../sarah_phase2/index.html">Open the Phase 2 coverage report</a>.</p>
    </section>"""
...
def phase3_panel() -> str:
    ...
    <section>
      ...
      <p class="note">These are one-variable cohort screens, not replacement-strategy backtests. <a href="../sarah_phase3/index.html">Open the Phase 3 screen report</a>.</p>
    </section>"""
...
def phase4_panel() -> str:
    ...
    <section>
      ...
      <p class="note">Every line was independently re-simulated through the one-minute strategy engine. <a href="../sarah_phase4/index.html">Open the Phase 4 report</a>.</p>
    </section>"""
...
def phase5_panel() -> str:
    ...
    <section>
      ...
      <p class="note">Combination promotion requires improvement over the first-touch core, not merely positive historical performance. <a href="../sarah_phase5/index.html">Open the Phase 5 report</a>.</p>
    </section>"""
...
def phase6_panel() -> str:
    ...
    <section>
      ...
      <p class="note">The Phase 5 candidate is frozen; historical stress passes, but code parity and forward evidence are still open. <a href="../sarah_phase6/index.html">Open the Phase 6 report</a>.</p>
    </section>"""
...
<title>Sarah strategy lab</title><style>{STYLE}</style></head><body><main class="wrap">
<div class="eyebrow">MNQ · Zone strategy research</div><h1>Sarah strategy lab</h1>
<p class="lead">A living comparison dashboard. The original Sarah rules remain the control line; each confluence experiment can be registered beside it using the same trade-ledger schema.</p>
...
<div class="method"><code>.venv/bin/python scripts/mnq/sarah_html_report.py --series "BOS filter=reports/sarah_bos/trades.parquet"</code>
...
<footer>Generated offline by <code>scripts/mnq/sarah_html_report.py</code>. Supporting CSVs and the persistent <code>series.json</code> registry are beside this file.</footer>
...
def main() -> None:
    ...
    print(f"Sarah dashboard: {len(series)} line(s) -> {output}")
