"""Screen Sarah confluence variables one at a time without changing trade mechanics.
...
Run from the repository root:
    ...
    .venv/bin/python scripts/mnq/sarah_phase3_screen.py
"""
...
from sarah_html_report import comparison_frames, svg_line_chart


ROOT = Path(__file__).resolve().parents[2]
DEFAULT_PHASE1 = ROOT / "reports" / "sarah_baseline"
DEFAULT_PHASE2 = ROOT / "reports" / "sarah_phase2"
DEFAULT_OUTPUT = ROOT / "reports" / "sarah_phase3"
...
def attach_features(
    ...
    executed = retests.loc[retests.sarah_trade_executed].copy()
...
def candidate_masks(frame: pd.DataFrame) -> dict[str, tuple[str, pd.Series]]:
    ...
    return {
        "Sarah baseline": ("All trades", pd.Series(True, index=frame.index)),
        ...
        "1h · first Sarah test": ("1h", one_hour & frame.test_number.eq(1)),
...
def bucket_definitions(frame: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    return {
        ...
        "Sarah test | first": frame.test_number.eq(1),
        "Sarah test | second": frame.test_number.eq(2),
...
def classify_experiments(table: pd.DataFrame) -> pd.DataFrame:
    ...
    for experiment, group in table.groupby("experiment", sort=False):
        ...
        if experiment in {"Sarah baseline", "1h reference"}:
            status = "reference"
        elif not enough:
...
def write_outputs(
    ...
    masks = candidate_masks(frame)
    chart_names = [
        "Sarah baseline", "1h reference", "1h · first Sarah test",
    ...
    baseline_pf = experiments.loc[
        (experiments.experiment == "Sarah baseline") & (experiments.split == "Overall"),
        "profit_factor",
    ].iloc[0]
    ...
    <title>Sarah Phase 3 confluence screen</title><style>{STYLE}</style></head><body><main class='wrap'>
    <div class='eyebrow'>MNQ · Sarah strategy lab</div><h1>Phase 3 confluence screen</h1>
    <p class='lead'>Each condition is tested separately on Sarah's unchanged baseline trades. Results are split chronologically and uncertainty is estimated by resampling calendar months.</p>
    ...
    <p class='note'><a href='../sarah_dashboard/index.html'>Main baseline dashboard</a> · <a href='../sarah_phase2/index.html'>Phase 2 feature report</a></p>
