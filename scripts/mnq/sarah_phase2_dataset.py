"""Build Sarah Phase 2 zone and retest research datasets.
...
Run from the repository root:
    ...
    .venv/bin/python scripts/mnq/sarah_phase2_dataset.py
"""
...
DEFAULT_OUTPUT = ROOT / "reports" / "sarah_phase2"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sarah-stop-cap", type=float, default=100.0)
...
def add_touch_features(
    ...
    # Associate Sarah's consumed tests with their enclosing physical touch episode.
    result["sarah_tests_in_episode"] = 0
    result["sarah_trade_executed"] = False
    result["sarah_not_executed_reason"] = None
    tests_by_zone = {
    ...
    for idx, row in result.iterrows():
        ...
        if len(inside):
            result.at[idx, "sarah_tests_in_episode"] = len(inside)
            result.at[idx, "sarah_trade_executed"] = bool(inside["executed"].any())
            reasons = sorted({str(value) for value in inside["not_executed_reason"].dropna()})
            result.at[idx, "sarah_not_executed_reason"] = ",".join(reasons) if reasons else None
...
def add_opposing_room(
    ...
    result["sarah_risk_points"] = structural_risk.clip(upper=args.sarah_stop_cap)
...
def write_outputs(
    ...
    feature_dictionary = {
        ...
        "structural_risk": f"Zone width plus {args.zone_stop_buffer:g} point; separate from Sarah's {args.sarah_stop_cap:g}-point capped risk.",
    ...
    overall = {
        ...
        "sarah_executed_episodes": int(retests.sarah_trade_executed.sum()),
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.55 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    <title>Sarah Phase 2 feature dataset</title><style>{style}</style></head><body><main>
    <div class='eyebrow'>MNQ · Sarah strategy lab</div><h1>Phase 2 feature dataset</h1>
    ...
    <h2>Retest reactions</h2><p class='note'>Independent physical touch episodes, including events Sarah did not trade.</p>
