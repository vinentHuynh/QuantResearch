"""Reproduce Sarah's default zone-retest baseline on MNQ one-minute data.
...
keeps Sarah's current mechanics--including fixed point targets, the 100-point
...
Default run from the repository root:
    ...
    .venv/bin/python scripts/mnq/sarah_baseline_backtest.py

To reproduce Sarah's currently configured date window while retaining earlier
bars as zone warm-up history:

    .venv/bin/python scripts/mnq/sarah_baseline_backtest.py \
        --start 2026-01-01T23:00:00Z --end 2026-05-30T21:00:00Z \
        --output-dir reports/sarah_baseline_2026_window
"""
...
DEFAULT_OUTPUT_DIR = ROOT / "reports" / "sarah_baseline"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--point-value", type=float, default=2.0,
                        help="MNQ dollars per index point; Sarah's NQ display uses 20")
...
def detect_zones(
    ...
    # Sarah processes 1h before 4h before Daily. Each new zone is prepended to
...
def simulate(
    ...
    for pos, timestamp in enumerate(index):
        ...
        # Activate zones in Sarah's timeframe call order, prepending each one.
        ...
        # Track physical touch episodes independently from Sarah's selected
        ...
        # Sarah updates rearming on every bar, but only selects tests when flat.
        if trade_was_active:
        ...
        # Date-range end cancels an open trade in Sarah without an outcome.
        ...
        if active_trade is not None:
            ...
            if can_exit and (stop_hit or target_hit or timed):
                ...
                if stop_hit:  # Sarah resolves an ambiguous bar against the trade.
    ...
    # Sarah normally closes intraday at 15:00 CT. Preserve a diagnostic if an
...
def write_report(
    ...
    summary = {
        "method": "Sarah default logic; 1-minute execution",
    ...
    report = f"""# Sarah MNQ baseline

This is Sarah's default 1h/4h/Daily two-candle zone logic executed on the MNQ
...
P&L uses MNQ at ${args.point_value:g}/point and {args.cost_ticks:g} round-trip cost ticks.

## Baseline caveats retained from Sarah
...
- If both sides qualify on one bar, Sarah consumes both tests and prioritizes the long.
...
def main() -> None:
    ...
    print("Simulating Sarah's retest engine on one-minute bars...", flush=True)
